...
void *AtenderCliente(void *socket)
{
    ...
    while (terminar == 0)
    {
        ...
        else if (codigo == 1) // Conversión de Celsius a Fahrenheit
        {
            ...
            sprintf(respuesta, "1/%d/%.2f Celsius son %.2f Fahrenheit", numForm, temperatura, fahrenheit);
        }
        else if (codigo == 2) // Conversión de Fahrenheit a Celsius
        {
            float celsius = (temperatura - 32.0) * 5.0 / 9.0;
            sprintf(respuesta, "2/%d/%.2f Fahrenheit son %.2f Celsius", numForm, temperatura, celsius);
        }
